...
$styleswitcher = '<select class="form-select form-select-sm" id="themes">' . "\n";
...
$xdebug_info = '';
	if (function_exists('xdebug_info')) {
		...
		$xdebug_info = '<li class="list-group-item" ><a href="?xdebuginfo">xdebug_info()</a></li>';
	}
...
$langueswitcher = '<form method="get" class="row g-2"><select class="form-select form-select-sm" name="lang" id="langues" onchange="this.form.submit();">' . "\n";
$selected = false;
...
<div id="mysqlmariadb" class="modalOto">
	<div>
		<div class='modalOtoBar'><input type='button' value='' class='js-copyc' data-target='none'>
			<a href="#closeOto" title="Closeb" class="closeOto">X</a>
		</div>
		{$langues['HelpMySQLMariaDB']}
	</div>
</div>
EOF;
	$popupMySQLMariaDBLink = "&nbsp;-&nbsp;<a href='#mysqlmariadb'><small style='color:#red;'>MySQL - MariaDB</small></a>";
		...
		if (is_file($aliasDir . $file) && strstr($file, '.conf')) {
			...
			if (stripos($file, 'phpmyadmin') !== false) {
				...
				foreach ($Alias_Contents['PMyAdVer'] as $key => $none) {
					...
					$aliasContents .= '<li class="list-group-item"><a href="' . $href . '/">' . $file . '</a>' . $file_sup . '</li>';
					$nbAlias++;
					$nbAliasLines++;
				...
				$aliasContents .= '<li class="list-group-item"><a href="' . $href . '/">' . $file . ' ' . $Alias_Contents['adminer']['version'] . '</a>' . $file_sup . '</li>';
				$nbAlias++;
				$nbAliasLines++;
			}
			...
			elseif (stripos($file, 'phpsysinfo') === false) {
				...
				$aliasContents .= '
				                    <li>
									<a href="' . $href . '/">' . $file . '</a>' . $file_sup . '</li>'
...
$addVhost = "<li>
                <a href='add_vhost.php?lang=" . $langue . "'>" . $langues['txtAddVhost'] . "</a>
			</li>";
					...
					if ($virtualHost['ServerNameValid'][$value] === false) {
						...
						if (!$virtualHost['port_listen'] && $virtualHost['ServerNamePortListen'][$value] !== true || $virtualHost['ServerNamePortApacheVar'][$value] !== true) {
							...
							if (isset($c_ApacheDefine['PHPROOT']) && $virtualHost['ServerNameFcgid'][$value] === true) {
								$value_aff .= "<p style='margin:-11px 0 -2px 25px;color:red;'><small>FCGI -> PHP " . $virtualHost['ServerNameFcgidPHP'][$value] . "</small></p>";
								$nbVirtualHostLines++;
							}
...
if (count($list_projects) > 0) {
	...
	foreach ($list_projects as $file) {
		$projectContents .= ($wampConf['LinksOnProjectsHomePage'] == 'off') ? "


		<span>

		<a class='btn btn-outline-primary' type='button' href='http://localhost/" . $file . "/'>" . $file . "</a>
...
if (empty($projectContents))
	...
	if ($wampConf['LinksOnProjectsHomePage'] == 'off') {
		...
		$projectContents .= "<p class='projectsdir'>" . sprintf($langues['txtProjects'] . '.<br>' . $langues['txtProjectsLink'], $wwwDir) . "</p>";
...
<div class="container">
	    ...
	    <nav class="navbar bg-body-tertiary">
			...
			<div class="container-fluid">
				...
				<p class="font-monospace pt-3">Apache 2.4 - MySQL 5 &amp; 8 - MariaDB 10 - PHP 5, 7 &amp; 8</p>

			   <span class="navbar-brand mb-0 h1">{$c_wampVersion} - {$c_wampMode}</span>
		...
		<div class="card">
			...
			<div class="column">
				...
				{$langues['versa']} {$apacheVersion}
				<a href='http://{$langues[$doca_version]}'>{$langues['documentation-of']} Apache</a>

				<h2>{$langues['server']}</h2>
				<p>{$server_software}&nbsp;-&nbsp;{$langues['portUsed']}{$ListenPorts}</p>

				<h2>{$langues['versp']}</h2>

		        <p>
					<small style='color:blue;'>[Apache module]</small>

					{$phpVersion}

					<a href='http://{$langues['docp']}'>{$langues['documentation-of']} PHP</a>
					
					{$popupPHPExtLink}

					{$PhpAllVersionsNotFcgi}

		        	<p><small style='color:green;'>[FCGI]</small> {$PhpAllVersions}</p>
